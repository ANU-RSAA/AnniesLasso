...
class TestCannonModelInit:
    ...
    def test_cannonmodel_init_mismatched_training_sets(
        ...
        training_set_flux = np.ones(training_set_flux_shape) if training_set_flux_shape is not None else None
        training_set_ivar = np.ones(training_set_ivar_shape) if training_set_ivar_shape is not None else None
        training_set_labels = np.ones(training_set_labels_shape) if training_set_labels_shape is not None else None

        with pytest.raises(ValueError, match=error_match):
            _ = model.CannonModel(training_set_labels, training_set_flux, training_set_ivar, vec)

    @pytest.mark.parametrize("training_set_shape", [(10,), (10, 10), (100, ), (100, 100), (10, 100)])
    @pytest.mark.parametrize("inf_posn", [0, 25, 50, 75, 99])
    class TestCannonModelInfValues(object):

        def test_cannonmodel_inf_flux(self, vectorizer, label_names, terms, training_set_shape, inf_posn):
            vec = vectorizer(label_names=label_names, terms=terms)
            training_set_flux = np.ones(training_set_shape)
            training_set_flux[int((inf_posn/100.0) * training_set_flux.shape[0])] = np.inf
        ...
        def test_cannonmodel_inf_ivar(self, vectorizer, label_names, terms, training_set_shape, inf_posn):
            vec = vectorizer(label_names=label_names, terms=terms)
            training_set_flux = np.ones(training_set_shape)
            training_set_ivar = np.ones(training_set_shape)
            training_set_ivar[int((inf_posn/100.0) * training_set_ivar.shape[0])] = np.inf
            training_set_labels = np.ones(training_set_shape)
