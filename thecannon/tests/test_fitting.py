...
def test_chisq_input_none(argnone):
    ...
    args = [None if k == argnone else dummy_array for k in ["theta", "design_matrix", "flux", "ivar"]]
...
@pytest.mark.parametrize("design_matrix", [
    np.ones((3, )),
    np.ones((2, 4)),  # Should not combine OK with same flux shape above - is transposed in calculation
])
@pytest.mark.parametrize("flux", [
    np.ones((3, )),
    np.ones((6, 4, 2)),
...
def test_chisq_input_bad_shapes(theta, design_matrix, flux):
    with pytest.raises(ValueError, match="inconsistent shapes"):
        _ = fitting.chi_sq(theta, design_matrix, flux, flux)  # Let shape of ivar == shape flux
