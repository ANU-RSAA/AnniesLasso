...
def _continuum_design_matrix(dispersion, L, order):
    ...
    try:
        ...
        assert dispersion.ndim == 1, "Dispersion must be one-dimensional"
    ...
    **kwargs
    ...
    for i in range(flux.shape[0]):
        ...
        if any(warn_indices):
            ...
            segments = ", ".join(
                [
                    "{:.1f} to {:.1f} ({:d} pixels)".format(
                        dispersion[s], dispersion[e], e - s
                    )
            ...
            logging.warn(
                "Some pixels in spectrum index {0} have measured flux "
