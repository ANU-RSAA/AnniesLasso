...
def fit(model_filename, spectrum_filenames, threads, clobber, from_filename, **kwargs):
    """
    ...
    for i, filename in enumerate(spectrum_filenames):
        ...
        if os.path.exists(output_filename) and not clobber:
            logger.info(
                "Output filename {} already exists and not clobbering.".format(
                    output_filename
                )
            )
            continue
        ...
        try:
            ...
            if len(output_filenames) >= chunk_size:
                ...
                for result, cov, meta, output_filename in zip(
                    results, covs, metas, output_filenames
                ):
...
def main():
    ...
    subparsers = parser.add_subparsers(
        title="action", dest="action", description="Specify the action to perform."
    )
    ...
    fit_parser.add_argument(
        "model_filename", type=str, help="The path of a trained Cannon model."
    )
    fit_parser.add_argument(
        "spectrum_filenames", nargs="+", type=str, help="Paths of spectra to fit."
    )
    fit_parser.add_argument(
        "--parallel-chunks",
        ...
        type=int,
        default=1000,
        help="The number of spectra to fit in a chunk.",
    )
    fit_parser.add_argument(
        "--clobber",
        dest="clobber",
    ...
    fit_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Read spectrum filenames from file",
    )
    ...
    join_parser.add_argument(
        "output_filename", type=str, help="The path to write the output filename."
    )
    join_parser.add_argument(
        "result_filenames",
        nargs="+",
